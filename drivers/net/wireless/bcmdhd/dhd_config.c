...
// SPKL SparkLAN Eric drv_acs
	else if (!strncmp("drv_acs=", full_param, len_param)) {
		if (!strncmp(data, "0", 1))
			conf->drv_acs = 0;
		else
			conf->drv_acs = 1;
		CONFIG_MSG("drv_acs = %d\n", conf->drv_acs);
	}
...
#else
//SparkLAN SPKL modify default country code
	else if(conf->chip == BCM43752_CHIP_ID) {
		strcpy(conf->cspec.country_abbrev, "XZ");
		strcpy(conf->cspec.ccode, "XZ");
		conf->cspec.rev = 0;
	}
	else {
			strcpy(conf->cspec.country_abbrev, "WW");
			strcpy(conf->cspec.ccode, "WW");
			conf->cspec.rev = 0;
	}
#endif
	memset(&conf->ccode_all.cspec, 0, sizeof(wl_country_t));
