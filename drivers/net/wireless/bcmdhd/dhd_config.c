...
const chip_name_map_t chip_name_map[] = {
	...
	{BCM4356_CHIP_ID,	2,	"bcm4356a2_pcie_ag",	""},
...
const chip_name_map_v2_t chip_name_map_v2[] = {
	...
	{BCM4612_CHIP_ID,	1,	"syn4612a1"},
	{BCM4612_CHIP_ID,	3,	"syn4612a3"},
...
dhd_conf_csa_chip(dhd_pub_t *dhd)
{
	...
	if (dhd->conf->csa >= 0)
		csa = dhd->conf->csa;
	else if (chip == BCM43430_CHIP_ID || chip == BCM4356_CHIP_ID ||
				chip == BCM4359_CHIP_ID || chip == BCM43569_CHIP_ID)
		csa = FALSE;
...
bool
dhd_conf_syna_secure_chip(dhd_pub_t *dhd)
{
	uint chip;

	chip = dhd->conf->chip;

	if (chip == BCM4612_CHIP_ID) {
		return TRUE;
	}
...
dhd_conf_legacy_pcie_chip(dhd_pub_t *dhd)
{
	...
	if (chip == BCM4345_CHIP_ID || chip == BCM4356_CHIP_ID ||
			chip == BCM4359_CHIP_ID) {
		return true;
	}
...
dhd_conf_read_pcie_params(dhd_pub_t *dhd, char *full_param, uint len_param)
{
	...
	else if (!strncmp("rpm=", full_param, len_param)) {
		if (!strncmp(data, "1", 1))
			conf->rpm = TRUE;
		else
			conf->rpm = FALSE;
...
dhd_conf_read_pm_params(dhd_pub_t *dhd, char *full_param, uint len_param)
{
	...
	if (!strncmp("deepsleep=", full_param, len_param)) {
		...
		uint val = (int)simple_strtol(data, NULL, 0);
		conf->insuspend |= val;
...
dhd_conf_set_intr_extn(dhd_pub_t *dhd)
{
	...
	if (chip == BCM43012_CHIP_ID ||
			...
			chip == BCM4356_CHIP_ID || chip == BCM4359_CHIP_ID ||
			chip == BCM43752_CHIP_ID || chip == BCM43756_CHIP_ID ||
			chip == BCM43711_CHIP_ID || chip == BCM4612_CHIP_ID ||
...
dhd_conf_postinit_ioctls(dhd_pub_t *dhd)
{
	...
	if (conf->chip == BCM43711_CHIP_ID || conf->chip == BCM4612_CHIP_ID ||
...
dhd_conf_preinit(dhd_pub_t *dhd)
{
	...
	if (conf->chip == BCM4345_CHIP_ID || conf->chip == BCM4359_CHIP_ID ||
		...
		conf->cspec.rev = 38;
			...
			conf->chip == BCM43711_CHIP_ID || conf->chip == BCM4612_CHIP_ID ||
...
//	strcpy(conf->ccode_all.cspec.ccode, "AA");
	...
	if (conf->chip == BCM43752_CHIP_ID)
		conf->war = SET_CHAN_INCONN | FW_REINIT_INCSA | FW_REINIT_EMPTY_SCAN |
			FW_REINIT_RXF0OVFL;
	else if (conf->chip == BCM43711_CHIP_ID || conf->chip == BCM4612_CHIP_ID)
