...
int dhd_idletime = MAX_IDLE_COUNT;
#else
...
#if defined(SHOW_LOGTRACE) && defined(WL_CFG80211)
static int
...
dhd_open(struct net_device *net)
{
	...
	int bytes_written = 0;
#endif /* WL_EXT_IAPSTA && ISAM_PREINIT */
...
dhd_attach_net(dhd_pub_t *dhdp, bool need_rtnl_lock)
{
	...
	(void)memcpy_s(primary_ndev->perm_addr, ETHER_ADDR_LEN,
		primary_ndev->dev_addr, ETHER_ADDR_LEN);
...
dhd_os_check_wakelock_all(dhd_pub_t *pub)
{
	...
	DHD_RPM(("%s wakelock c-%d wl-%d wd-%d rx-%d "
	...
	size_t count, loff_t *ppos)
{
	...
	if (!g_d2m_dbgfs.memdump_size && g_d2m_dbgfs.dhdp)
		g_d2m_dbgfs.memdump_size = g_d2m_dbgfs.dhdp->soc_ram_length;

	if (pos < 0)
		return -EINVAL;
	if (pos >= g_d2m_dbgfs.memdump_size || !count) {
		/* end of memory dump, clear flag and return  */
		DHD_ERROR(("#DHD memory dump ended : %lld\n", pos));
	...
	ret = copy_to_user(ubuf, (void *)((uintptr_t)g_d2m_dbgfs.dhdp->soc_ram + (*(int *)ppos)),
			count);
	if (ret) {
		DHD_ERROR(("#DHD memory dump left %zu bytes : %zu\n", count, ret));
	}
...
dhd_mem_dump(void *handle, void *event_info, u8 event)
{
	...
	if (write_dump_to_file(&dhd->pub, dump->buf, dump->bufsize, "mem_dump")) {
		...
		DHD_ERROR(("%s: DATA dump to memory from kernel =0x%x,memdump_type=%d\n",
			__FUNCTION__, atomic_read(&g_d2m_dbgfs.memdump_active), memdump_type));
#endif
	}
