	...
	wl_event_msg_t *event, void **data_ptr, void *raw_event)
{
	...
	case WLC_E_LINK:
		...
		if (flags) {
			...
			if (rc == BCME_OK) {
				DHD_INFO(("WLC_E_LINK: idx:%d, action:%s, "
					"iftype:%s, ["MACDBG"]\n", ifindex,
					((flags) ? "UP":"DOWN"),
...
#ifdef SHOW_LOGTRACE
int
dhd_parse_logstrs_file(dhd_pub_t *dhd, char *raw_fmts, int logstrs_size,
