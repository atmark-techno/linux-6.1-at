...
#ifdef WL_EXT_IAPSTA
			&& !wl_ext_iapsta_other_if_associated(dev)
#endif /* WL_EXT_IAPSTA */
			) {
		err = wldev_ioctl_set(dev, WLC_DOWN, &ap, sizeof(s32));
		if (err < 0) {
	...
	int static_ifidx)
{
	...
	if (iface_wlan1_mac[0]) {
...
#endif
#ifdef CUSTOM_MULTI_MAC
	else if (!wifi_platform_get_mac_addr(dhd->adapter, hw_ether, static_ifidx+1)) {
		...
		mac_addr[0] |= 0x02;
#ifdef SPECIFIC_MAC_GEN_SCHEME
		wl_ext_get_vif_macaddr(primary_ndev, WL_IF_TYPE_AP, static_ifidx, mac_addr);
#endif /* SPECIFIC_MAC_GEN_SCHEME */
	}
...
#ifdef CUSTOM_MULTI_MAC
		else if (!wifi_platform_get_mac_addr(dhd->adapter, hw_ether,
				static_ifidx+1)) {
